...
- **Tareas**: solo `codigo` funciona en el pipeline (el seed solo tiene modelos de código). Si se solicita `chat` o `razonamiento`, el pipeline devuelve **una lista de candidatos vacía y sin ningún mensaje**: `ejecutar_pipeline(tarea="chat")` → 0 candidatos, igual que `razonamiento`, y `POST /chat` responde `tipo: "recomendacion"` con `candidatos: []`. Es decir, se obtiene precisamente el resultado vacío ambiguo que este documento exige evitar. El aviso explícito ("el catálogo para esas tareas aún no está sincronizado **en el pipeline**") está **pendiente de implementar**: no existe ningún código que lo produzca. El bootstrap ya contiene modelos de esas tareas, pero no está conectado.
...
- **La fuente no expone idiomas**: el contrato observado de `GET /api/models` solo tiene `id`, `name`, `provider`, `family`, `params`, `paramsBillions`, `architecture`, `releaseDate`, `contextLength`, `useCase`, `url` y `license` — **no hay campo de idiomas**. Por eso `normalizer.py` fija `idiomas_detectados=None` en todos los registros sincronizados. Consecuencia: el filtro por `idiomas_requeridos` solo tiene efecto sobre el catálogo semilla (que sí declara idiomas); cuando el pipeline cargue el snapshot sincronizado, ese filtro dejará de descartar nada.
- **El runtime no se filtra**: `runtime_preferido` se acepta en la petición y se pasa a `CanIRunProvider.check(...)`, pero ese método no lo lee. El scorer tampoco tiene criterio de runtime (`PESOS`: `ajuste_vram`, `evidencia_codigo`, `documentacion`, `tamanio`, `licencia`). En la práctica el runtime de la petición no influye en el resultado.
...
- El ranking solo puede usar datos estructurados y deterministas disponibles en el snapshot y en la comprobación de compatibilidad: tarea, compatibilidad VRAM, licencia, idiomas y tamaño/contexto si la fuente los proporciona. Concretamente hoy el scorer lee `pipeline_tag`, `license_declarada`, `idiomas_detectados`, `vram_required_gb` y `tokens_per_second` (`PESOS` en `src/ranking/scorer.py`); el subcriterio `documentacion` sigue siendo un marcador de posición, y `runtime_preferido` y `contexto_maximo` se aceptan en el esquema pero no se usan.
